...
function runSG2(){
  ...
  const genderPNJ2 = female?'Mujer':'Hombre';
  ...
  SG_LAST={b,el,yos,arcKey,A,s,nm,mean,house,school,jutsus,gskl,advPicks,gender:genderPNJ2,
...
(function(){
  ...
  if($('#monGo')) $('#monGo').addEventListener('click',()=>{
    ...
    else if(pc<=30){name=`${cjp} ${pick(MMAT)} ${pick(MPLACE)}`}
    else{name=`${pick(S1)+pick(S2)}, ${pick(TITLE)}`}
    const size=pc<=5?pick(['pequeño','mediano']):pc<=10?pick(['mediano','mediano','grande']):pc<=15?pick(['mediano','grande']):pc<=25?pick(['grande']):pc<=35?pick(['grande','colosal']):pick(['colosal']);
    const arch=pc<10?pick(['bruto','veloz','tanque']):pick(['bruto','tanque','veloz','chamán']);
    let ju=[];
    if(arch==='chamán'||(pc>=20&&Math.random()<.35)){
      const n=pc>=35?2:1;
      const lvmax=pc<=5?1:pc<=10?2:pc<=15?2:pc<=25?3:pc<=35?4:4;
  ...
  if($('#pnjGo')) $('#pnjGo').addEventListener('click',()=>{
    ...
    const col=ELC[yos[0]];
    const houseMean2 = typeof KAZOKU_MEANING!=='undefined' && KAZOKU_MEANING[house] ? ` (${KAZOKU_MEANING[house]})` : '';
    const physPNJ=`Pelo ${pick(SG_HAIR_C)} ${pick(SG_HAIR_S)}, ojos ${pick(SG_EYE)}, ${pick(SG_BOD).toLowerCase()}. Viste ${ELCOL[yos[0]]||'colores de su Kazoku'} y ${pick(SG_TREND).toLowerCase()}.`;
    const genderPNJ = female?'Mujer':'Hombre';
    $('#pnjOut').innerHTML=`
      <h4>${nm} <small style="font:400 13px var(--fb);color:#6f6350">(${mean})</small> <span style="font:600 11px var(--fpx);color:#8a6b2e;border:1px solid #cdbf9d;border-radius:999px;padding:1px 8px;margin-left:6px">${genderPNJ}</span></h4>
      <p class="gsub">${yos[1]} · Kazoku ${house}${houseMean2} · Escuela ${school} · ${an} (${pc} PC) · ${genderPNJ}</p>
      <p class="gline" style="margin:6px 0"><b>Físico:</b> ${physPNJ}</p>
      <table class="tb" style="--c:${col};margin-top:6px">
        <tr><th>In</th><th>At CC</th><th>Daño CC</th><th>At Dis</th><th>Daño Dis</th><th>Def CC</th><th>Def Dis</th><th>Abs</th><th>PV</th><th>CK</th><th>PK</th></tr>
      ...
      <p class="gline"><b>Habilidades completas (${pc} PH):</b> ${skills}</p>
  ...
  /* cap06: generador de Yosai completo */
  if($('#ygGo')) $('#ygGo').addEventListener('click',()=>{
    const yv=$('#ygYos').value, sz=$('#ygSize').value;
    const yos=yv==='any'?pick(YOS):YOS.find(y=>y[0]===yv);
    const el=yos[0];
    const houses = [...HOUSES[el]];
    // Festivals and capitals invented
    const caps={hi:'Kazan-no-Miya',ts:'Iwakura',kz:'Kazekaze',mz:'Minato',tg:'Tengudake',ym:'Shinkai',kn:'Kinzoku-no-Kama',kr:'Kōri-no-Shiro',ik:'Ikazuchi-no-Tō',ar:'Arashi-no-Toride',mr:'Mori-no-Sato',yu:'Yume-no-Hakkyō'};
    const shoguns={hi:'Hi no Akane',ts:'Raion Haruto',kz:'Washi Haru',mz:'Same Jin',tg:'Amateru',ym:'Kobura Ren',kn:'Kinzoku Hagane',kr:'Kōri Yukihime',ik:'Ikazuchi Raikō',ar:'Arashi Hayate',mr:'Mori Kodama',yu:'Yume Baku'};
    const nKaz = sz==='compact'?5+Math.floor(Math.random()*2):sz==='grande'?9+Math.floor(Math.random()*2):7+Math.floor(Math.random()*2);
    // Pick houses for this Yosai: take all majors + some random minors
    const majors=houses.slice(0,5);
    const minors=houses.slice(5);
    const chosen = [...majors];
    while(chosen.length < nKaz && minors.length){
      const h = minors.splice(Math.floor(Math.random()*minors.length),1)[0];
    ...
    const yosHist={
      hi:'Volcán extinto y bosque de cenizas. Shogun viudo busca alianzas. Risu quiere casarse con Kitsune.',
      ts:'Llanuras y río Aoi. Kotei Tsuchi preside la corte. Raion y Okami disputan el voto.',
      kz:'Ciudad escalón de 7 niveles. Washi y Hayabusa compiten por la altura.',
      mz:'Canales y puerto. Same y Mekajiki disputan la pesca.',
      tg:'Cima nevada del Tengudake, sede del Tenno. Ryu guarda el Espejo del Cielo.',
      ym:'Ciudad hundida Shinkai. Consejo de la Venganza planea volver.',
      kn:'Forja eterna de Kinzoku. Tetsu y Hagane compiten por el acero negro. Inventado para esta edición.',
      kr:'Palacio de hielo de Kōri. Yukihyo y Tōketsu guardan el invierno eterno. Inventado.',
      ik:'Torre del Rayo de Ikazuchi. Raikō e Inazuma canalizan la tormenta. Inventado.',
      ar:'Fortaleza de la Tormenta Arashi. Shippu y Hayate cabalgan el viento. Inventado.',
      mr:'Aldea oculta del Bosque Mori. Kodama y Mokujin hablan con los árboles. Inventado.',
      yu:'Palacio onírico de Yume. Baku y Yumekui devoran pesadillas. Inventado.'
    };
    let out=`<h4>${yos[1]} — ${caps[el]||'Capital'} · Shogun ${shoguns[el]||''}</h4>
      <p class="gsub">Yosai de ${ELM[el][1]} ${ELM[el][0]} · ${houses.length} Kazokus en el libro, aquí ${chosen.length} generadas · ${yosHist[el]||''}</p>
      <p class="gline"><b>Historia breve:</b> ${pick(['Fundado tras la caída de Ichikoku','Forjado en la guerra contra Kobura','Nacido de un pacto entre Daimyos','Levantado sobre ruinas de la ciudad-templo','Bautizado por el primer Shogun en la cima de su elemento'])} y marcado por ${pick(['una hambruna','una traición','un torneo amañado','una boda fallida','un monstruo del bestiario'])}. Hoy ${pick(['prospera con dudas','se rearme en silencio','busca alianzas matrimoniales','sufre la sequía','celebra el Matsuri'])}.</p>`;
    chosen.forEach((house,i)=>{
      const daimyo = pick([...NF,...NM]);
      const spec = pick(['jutsus de fuego que ignoran absorción','infiltración y sigilo','supervivencia y rastreo','duelo y forja','navegación y comercio','rituales y etiqueta']);
      const mean = typeof KAZOKU_MEANING!=='undefined' && KAZOKU_MEANING[house] ? ` (${KAZOKU_MEANING[house]})` : '';
      out+=`<div class="card" style="--c:${ELC[el]};margin:10px 0"><h4>${house}${mean} <span style="font:400 12px var(--fb);color:#6f6350">· Daimyo ${daimyo[0]} (${daimyo[1]})</span></h4><p class="sub">Especialidad: ${spec} · ${i<5?'Mayor':'Menor'} · Consejo ${i<5?'con voto':'sin voto'}</p>`;
      // 2-3 PNJs per house — con bloque completo como en cap13
      const nPnj = 2 + Math.floor(Math.random()*2);
      for(let k=0;k<nPnj;k++){
        const pc = pick([10,10,20]);
        const [an,s]=pick(TPL[pc]||TPL[10]);
        const female=Math.random()<0.5;
        const [nm,mean]=pick(female?NF:NM);
        const school=pick(['Kage','Reikon','Shisen']);
        const pool=school==='Kansei'?[...SKP.Kage,...SKP.Reikon,...SKP.Shisen]:SKP[school];
        const skills=pickN(pool,2).map(x=>`${x} ${1+Math.floor(Math.random()*2)}`).join(', ');
        const colP=ELC[el];
        const genderPNJ = female?'Mujer':'Hombre';
        const physPNJ=`Pelo ${pick(SG_HAIR_C)} ${pick(SG_HAIR_S)}, ojos ${pick(SG_EYE)}, ${pick(SG_BOD).toLowerCase()}.`;
        const maxLv=pc===10?2:3;
        const js=N(s.ju)>0?pickJutsus(maxLv,N(s.ju),el).map(j=>`<div class="gjs" style="margin:4px 0 0">${jutsuLine(j)}</div>`).join(''):'<div class="gjs" style="margin:4px 0 0"><i>Sin jutsu</i></div>';
        out+=`<div class="card" style="--c:${colP};margin:8px 0;padding:12px"><h4 style="margin:0">${nm} <small style="font:400 11px var(--fb);color:#6f6350">(${mean})</small> <span style="font:600 10px var(--fpx);color:#8a6b2e;border:1px solid #cdbf9d;border-radius:999px;padding:1px 6px;margin-left:6px">${genderPNJ}</span></h4>
          <p class="sub" style="margin:4px 0">${an} · ${pc} PC · ${school} · ${skills}</p>
          <p class="sub" style="margin:0 0 6px"><b>Físico:</b> ${physPNJ}</p>
          <table class="tb" style="--c:${colP};margin:6px 0"><tbody><tr><th>In</th><th>At Me</th><th>Da Me</th><th>At Di</th><th>Da Di</th><th>De Me</th><th>De Di</th><th>Abs</th><th>PV</th><th>CK</th><th>PK</th></tr><tr><td>+${N(s.in)}</td><td>+${N(s.atm)}</td><td>${dmgS(s.dtm)}</td><td>+${N(s.atd)}</td><td>${dmgS(s.dtd)}</td><td>${N(s.dfm)}</td><td>${N(s.dfd)}</td><td>${N(s.ab)}</td><td>${N(s.pv)}</td><td>+${N(s.ck)}</td><td>${N(s.pk)||'—'}</td></tr></tbody></table>
          ${js}
          <p class="sub" style="margin:6px 0 0"><b>Rasgo:</b> ${pick(QK)} · <b>Secreto:</b> ${pick(SEC)}</p></div>`;
      }
      out+=`</div>`;
    });
    out+=`<p class="sub" style="margin-top:8px">Generado con todas las familias del Yosai. Usa el buscador del navegador (Ctrl+F) para filtrar Kazokus, y “Cargar” PNJs sueltos en el constructor de cap08 si quieres afinar stats.</p>`;
    $('#ygOut').innerHTML=out;
  });
  ...
  if($('#sgGo')){
    ...
    $('#sgOut').addEventListener('click',e=>{
      ...
      const hm3 = typeof KAZOKU_MEANING!=='undefined' && KAZOKU_MEANING[c.house] ? ` (${KAZOKU_MEANING[c.house]})` : '';
      $('#bName').value=c.nm;$('#bKaz').value=`${c.house}${hm3} (${c.yos[1]})`;$('#bCon').value=c.concept;
      $('#bKi').value=ki;$('#bKiRow').style.display=ki==='none'?'none':'flex';
